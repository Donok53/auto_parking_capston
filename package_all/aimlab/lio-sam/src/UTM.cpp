...
void WGS84ToENU(GeoPoint *geopoint_ref_p,
                GeoPoint *geopoint_p,
                ENUPoint    *point_p)
{
    // Compute ZoneNumber
    double Lat = geopoint_p->lat;
    double Long = geopoint_p->lon;

    double LongTemp = (Long+180)-int((Long+180)/360)*360-180;

    double LatRad = Lat / 180.0 * M_PI;
    double LongRad = LongTemp / 180.0 * M_PI;
    ...
    double ref_x, ref_y;
    LatLonToUTMXY(geopoint_ref_p->lat, geopoint_ref_p->lon, ZoneNumber, ref_x, ref_y);
    double x, y;
    LatLonToUTMXY(geopoint_p->lat, geopoint_p->lon, ZoneNumber, x, y);
    ...
    point_p->x = local_x;
    point_p->y = local_y;
    point_p->z = geopoint_p->height - geopoint_ref_p->height; 
    return;
}

void WGS84ToENU(GeoPoint *geopoint_p,
                ENUPoint    *point_p)
{
    // Compute ZoneNumber
    double Lat = geopoint_p->lat;
    ...
    point_p->x = local_x;
    point_p->y = local_y;
    point_p->z = geopoint_p->height; 
    return;
}

void ENUToWGS84(GeoPoint *geopoint_ref_p,
                ENUPoint *point_p,
                GeoPoint *geopoint_p)
{
    // Compute ZoneNumber
    double Lat = geopoint_ref_p->lat;
    double Long = geopoint_ref_p->lon;
    ...
    // Reference x, y
    double x, y;
    LatLonToUTMXY(Lat, Long, ZoneNumber, x, y);

    double utmX_m = point_p->x + x;
    double utmY_m = point_p->y + y;
    double dLat_rad, dLon_rad;
    UTMXYToLatLon(utmX_m, utmY_m, ZoneNumber, false, dLat_rad, dLon_rad);

    geopoint_p->lat = dLat_rad;
    geopoint_p->lon = dLon_rad;
    geopoint_p->height = geopoint_ref_p->height + point_p->z; 
    return;
}

// DegToRad
// Converts degrees to radians.
FLOAT_t DegToRad(FLOAT_t deg) {
  return (deg / 180.0 * pi_t);
}


// RadToDeg
// Converts radians to degrees.
FLOAT_t RadToDeg(FLOAT_t rad) {
  return (rad / pi_t * 180.0);
}
...
FLOAT_t ArcLengthOfMeridian (FLOAT_t phi) {
  FLOAT_t alpha, beta, gamma, delta, epsilon, n;
  FLOAT_t result;
...
FLOAT_t UTMCentralMeridian(int zone) {
  FLOAT_t cmeridian;
  cmeridian = DegToRad(-183.0 + ((FLOAT_t)zone * 6.0));
...
FLOAT_t FootpointLatitude(FLOAT_t y) {
  FLOAT_t y_, alpha_, beta_, gamma_, delta_, epsilon_, n;
  FLOAT_t result;
...
void MapLatLonToXY (FLOAT_t phi, FLOAT_t lambda, FLOAT_t lambda0, FLOAT_t &x, FLOAT_t &y) {
    FLOAT_t N, nu2, ep2, t, t2, l;
    FLOAT_t l3coef, l4coef, l5coef, l6coef, l7coef, l8coef;
    //FLOAT_t tmp; // Unused
...
void MapXYToLatLon (FLOAT_t x, FLOAT_t y, FLOAT_t lambda0, FLOAT_t& phi, FLOAT_t& lambda)
{
  FLOAT_t phif, Nf, Nfpow, nuf2, ep2, tf, tf2, tf4, cf;
  FLOAT_t x1frac, x2frac, x3frac, x4frac, x5frac, x6frac, x7frac, x8frac;
  FLOAT_t x2poly, x3poly, x4poly, x5poly, x6poly, x7poly, x8poly;
...
int LatLonToUTMXY (FLOAT_t lat, FLOAT_t lon, int zone, FLOAT_t& x, FLOAT_t& y) {
  if ( (zone < 1) || (zone > 60) )
...
// Returns:
// The function does not return a value.
void UTMXYToLatLon (FLOAT_t x, FLOAT_t y, int zone, bool southhemi, FLOAT_t& lat, FLOAT_t& lon) {
  FLOAT_t cmeridian;
  ...
  MapXYToLatLon (x, y, cmeridian, lat_rad, lon_rad);

  lat = RadToDeg(lat_rad);
  lon = RadToDeg(lon_rad);
  return;
}
